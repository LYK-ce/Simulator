#include "Device_GPU.hh"
#include <iostream>

namespace AstraSim {

// 构造函数：初始化基类 + 初始化自己的成员
GPUDevice::GPUDevice(AstraNetworkAPI* NI, 
                     AstraMemoryAPI* MEM,
                     int id,
                     int npu_offset,
                     int num_passes,
...
{
    this->vram_bytes = vram_bytes;
    this->tflops = tflops;
    this->nvlink_lanes = nvlink_lanes;
  // 构造函数体：可以添加日志或额外初始化
  std::cout << "GPUDevice " << id << " created for CUDA device " 
            << std::endl;
}
